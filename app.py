...
llm_chain = None
try:
    GROQ_API_KEY = os.getenv("GROQ_API_KEY", "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")
    if not GROQ_API_KEY:
...
async def process_llm_and_tts(user_id: str, audio_filepath: str):
    ...
    try:
        ...
        # 2️⃣ Prepare for sentence buffering
        buffer = ""
        sentence_splitter = re.compile(r'([.!?])')

        # LLM chain without STT
        llm_only_chain = prompt | llm | parser

        # 3️⃣ Stream from LLM
        async for chunk_text in llm_only_chain.astream({"input": transcription}):
            if chunk_text:
                buffer += chunk_text
                await manager.send_json({'type': 'llm_chunk', 'text': chunk_text}, user_id)

                # Try splitting into complete sentences
                parts = sentence_splitter.split(buffer)
                # If we have more than 2 parts, at least one complete sentence exists
                while len(parts) >= 3:
                    sentence = (parts[0] + parts[1]).strip()
                    if sentence:
                        tts_queue.put(sentence)
                        logging.debug(f"Sent to TTS queue: '{sentence}'")
                    buffer = "".join(parts[2:])
                    parts = sentence_splitter.split(buffer)

        # 4️⃣ Flush remaining text (even without punctuation)
        if buffer.strip():
            tts_queue.put(buffer.strip())
            logging.debug(f"Sent remaining to TTS queue: '{buffer.strip()}'")

        # 5️⃣ End-of-response marker
